...
export function uiModeDescription(mode: UiMode, ctx: UiModeContext): string {
  if (ctx === 'trainer') {
    switch (mode) {
      case 'starter':
        return 'CRM, календарь, шаблоны и каталог. Серии и карта нагрузки. Без ИИ, ростера (атлеты/группы), чатов и лидерборда.';
      case 'pro':
        return 'CRM, ИИ, атлеты и группы, чаты, лидерборд, шаблоны и каталог. Без сложной аналитики (ATL/CTL, ACWR) и видеозвонков.';
      case 'unleash':
    ...
    switch (mode) {
      case 'starter':
        return {
          ...
          title: 'Минимум',
          subtitle:
            'CRM, календарь и назначения без лишнего: без AI, аналитики и работы с командой на старте — меньше отвлечений, пока осваиваетесь.',
        };
      case 'pro':
        return {
          emoji: '⚡',
          title: 'Рабочий набор',
          subtitle: 'CRM, AI, атлеты и группы, чаты — нормальный набор для ведения клиентов.',
