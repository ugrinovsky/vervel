...
function zoneStrokeColor(intensity: number): string {
  const hue = intensity <= 0.5 ? 140 - intensity * 2 * 92 : 48 - (intensity - 0.5) * 2 * 53;
...
const BodySVG: React.FC<BodySVGProps> = ({
  ...
  const getGlowFill = (appZone: string) => zoneColor(getZoneIntensity(appZone), 0.42);
  ...
  return (
    <div className="avatar-wrapper glass rounded-2xl relative w-full" style={{ paddingBottom: 0 }}>
      {/* Ambient radial glow */}
      <div
                      ...
                      {(isHigh || isMedium) &&
                        paths.map((d, i) => (
                          <motion.path
                            ...
                            initial={{ opacity: 0 }}
                            animate={
                              isHigh ? { opacity: [0, 0.65, 0.32, 0.65, 0] } : { opacity: 0.38 }
                            }
                            transition={
                              isHigh
                                ? {
