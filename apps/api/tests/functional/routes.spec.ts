...
test.group('AI routes: защита (401 без авторизации)', () => {
  const routes = [
    ...
    { method: 'get', url: '/ai/chat/messages' },
    { method: 'delete', url: '/ai/chat/messages' },
...
test.group('AI routes: доступ авторизованному пользователю', (group) => {
  group.setup(async () => {
  ...
  test('GET /ai/chat/messages → 200 и массив data', async ({ client, assert }) => {
    const user = await athleteUser()
    const response = await client.get('/ai/chat/messages').loginAs(user)
  ...
  test('DELETE /ai/chat/messages → 200', async ({ client }) => {
    const user = await athleteUser()
    const response = await client.delete('/ai/chat/messages').loginAs(user)
    response.assertStatus(200)
    response.assertBodyContains({ success: true })
  })
})
...
test.group('OAuth: защита и базовая логика', () => {
  ...
  test('GET /oauth/vk/redirect → 3xx на VK authorize, не 500', async ({ client, assert }) => {
    ...
    // In CI/prod without OAuth env, API returns 503 (provider not configured).
    assert.oneOf(response.status(), [301, 302, 303, 307, 308, 503])
  ...
  test('POST /oauth/set-role без авторизации → 401', async ({ client }) => {
