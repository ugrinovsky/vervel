...
const WorkoutsController = () => import('#controllers/workouts_controller')
...
router
  .delete('/push/unsubscribe', '#controllers/push_controller.unsubscribe')
  .use(middleware.auth())
...
router
  .group(() => {
    ...
    router.get(
      ...
      '#controllers/workouts_controller.getByScheduledId'
    )
    ...
    router.get(
      ...
      '#controllers/profile_controller.getTrainerPublicProfile'
    )
    ...
    router.put(
      'progression/strength-log/pins',
      '#controllers/progression_controller.putStrengthLogPins'
    )
    router.get(
      'progression/exercise-standards',
      '#controllers/progression_controller.getExerciseStandards'
    )
    router.post(
      'progression/exercise-standards',
      '#controllers/progression_controller.postExerciseStandard'
    )
    router.patch(
    ...
    router.get(
      ...
      '#controllers/athlete_controller.getOrCreateGroupChat'
    )
    router.get(
      'athlete/chats/trainer/:trainerId',
      '#controllers/athlete_controller.getOrCreatePersonalChat'
    )
    router.get(
      'athlete/groups/:id/leaderboard',
      '#controllers/progression_controller.getGroupLeaderboard'
    )

    // Dialogs list + shared chat actions (trainer or athlete participant)
    router.get('chats', '#controllers/chat_controller.listChats')
    router.get('chats/klipy/status', '#controllers/chat_controller.klipyStatus')
    router.get('chats/klipy/categories', '#controllers/chat_controller.listKlipyCategories')
    router.get('chats/klipy/search', '#controllers/chat_controller.searchKlipy')
...
router
  .group(() => {
    ...
    router.get(
      'athletes/:athleteId/periodization',
      '#controllers/trainer_controller.getAthletePeriodization'
    )
    ...
    router.delete(
      'groups/:id/athletes/:athleteId',
      '#controllers/trainer_controller.removeAthleteFromGroup'
    )
    ...
    // Passes (абонементы)
    router.get('passes', '#controllers/trainer_passes_controller.listAll')
    router.get('athletes/:athleteId/passes', '#controllers/trainer_passes_controller.list')
    router.post('athletes/:athleteId/passes', '#controllers/trainer_passes_controller.create')
    router.patch('passes/:id', '#controllers/trainer_passes_controller.update')
    router.post('passes/:id/usages', '#controllers/trainer_passes_controller.consume')
    router.delete('pass-usages/:usageId', '#controllers/trainer_passes_controller.deleteUsage')
  })
  .prefix('trainer')
