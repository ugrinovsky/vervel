...
export class WorkoutCalculator {
  ...
  private static calculateBodybuildingLoad(
    ...
    input.sets.forEach((set: WorkoutSet) => {
      ...
      const explicit =
        loggedWeight !== null && loggedWeight !== undefined && loggedWeight > 0
          ? loggedWeight
          : null

      const weightForVolume = explicit ? explicit : input.bodyweight ? (userBodyWeight ?? 0) : 0
  ...
  private static applyRpe(
    ...
    if (rpe === null || rpe === undefined) return { zonesLoadAbs, totalIntensity }
    const n = Number(rpe)
  ...
  static async calculateRecoveryState(
    ...
    const zones: Record<
      string,
      { intensity: number; lastTrainedDaysAgo: number; peakLoad: number }
    > = {}
    ...
    workouts.forEach((workout, i) => {
      ...
      const intensity =
        typeof workout.totalIntensity === 'string'
          ? Number.parseFloat(workout.totalIntensity)
        ...
        w.exercises.some(
          (ex) =>
            ...
            (ex.sets ?? []).some((s) => {
              ...
              return reps > 0 && (weight === null || weight === undefined || Number(weight) === 0)
            })
        )
