...
- zones: 1–4 зоны из [${ZONES_LIST}] (как в других режимах ИИ).
- zoneWeights: объект, ключи = те же строки что в zones; значения — доли нагрузки >0, сумма ровно 1. Главный драйвер движения (напр. ягодицы в румынской тяге) — самая большая доля; второстепенные мышцы — меньше. Указывай вместе с zones в каждом упражнении.`
...
11. zoneWeights — объект с теми же ключами что и zones; положительные доли, сумма = 1. У румынской тяги "glutes" обычно выше остальных; у жима лёжа больше "chests". Обязательно в каждом exercise вместе с zones.
    ...
    const exercises: AiExercise[] = (Array.isArray(parsed.exercises) ? parsed.exercises : []).map(
      (ex: any) => {
        ...
        if (
          ...
          if (Object.keys(rec).length > 0) zoneWeights = rec
        }
