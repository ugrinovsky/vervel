...
export class YandexAiService {
  /**
   * Генерирует короткое сообщение тренера атлету о назначенном плане.
   */
  static async generateCopilotMessage(params: {
    tsb: number
    phase: string
    overloadedZones: string[]
    daysSinceLastWorkout: number
    suggestedDatesLabel: string // напр. "понедельник 12 мая, среда 14 мая"
  }): Promise<string> {
    const apiKey = env.get('YANDEX_CLOUD_API_KEY')!
    const folderId = env.get('YANDEX_FOLDER_ID')!
    const model = env.get('YANDEX_GPT_MODEL', 'yandexgpt-lite')

    const tsbLabel =
      params.tsb < -10 ? 'высокая усталость' : params.tsb > 10 ? 'хорошая готовность' : 'норма'

    const systemPrompt = `Ты — тренер. Напиши короткое сообщение атлету о назначенном плане тренировок на неделю. Только текст, без кавычек, без форматирования. 2–3 предложения, дружелюбно, на «ты».`

    const userMessage = `Запланированные тренировки: ${params.suggestedDatesLabel}
Состояние атлета: фаза ${params.phase}, TSB ${params.tsb} (${tsbLabel}), дней без тренировки: ${params.daysSinceLastWorkout}${params.overloadedZones.length > 0 ? `, перегружены: ${params.overloadedZones.join(', ')}` : ''}`

    return this.callGpt(folderId, systemPrompt, userMessage, 0.7, apiKey, model, 300, {
      step: 'copilot.message',
    })
  }
