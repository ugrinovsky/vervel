...
const SUGGEST_STANDARD_LINKS_PROMPT = `Сопоставь названия из журнала (candidates) с эталонами (standards). Каталога упражнений нет — только строки exerciseName и displayLabel.

Как решать, одно ли это упражнение (делай это мысленно для каждой пары, в ответ не выводи):
1) Какой главный паттерн движения: жим от груди/плеч, тяга к корпусу, разгибание/сгибание в локте или колене, сгибание/разгибание бедра, подъём на носки, разведение в стороны, пуловер и т.д.?
2) Положение тела и направление усилия: стоя, сидя, лёжа на скамье (грудь вверх), наклон корпуса, над головой и т.д.?
3) Снаряд и вариант (гантели/штанга/тренажёр/резинка), если это явно из названия.

Связывай кандидата с эталоном только если совпадают и паттерн, и контекст (положение/угол/снаряд там, где они указаны в названии). Общие слова («гантели», «жим», «тяга», «наклон», «штанга») сами по себе НЕ доказательство совпадения: разные упражнения часто делят одно слово.

Если паттерн или контекст расходятся — не связывай, даже если мышечные группы пересекаются.
...
Правила вывода:
- sourceExerciseId — ТОЧНО как в candidates, посимвольно.
- standardId — только число из standards.
- В links только уверенные совпадения; сомнительные опусти.
- Нет подходящих пар — {"links":[]}
- Не придумывай id и числа вне входа.`
