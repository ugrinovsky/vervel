...
10b. «3х15 каждой ногой», «3х15 на каждую ногу» — это 3 подхода по 15 повторов (не 1). Вес относится к каждому подходу, если указан.
11. Кардио: duration в минутах; силовые/кроссфит — reps/setData.
12. notes — по-русски.
12b. СМЕШАННЫЙ ТРЕНИНГ: если в тексте есть силовые упражнения (повторы/вес) и отдельной строкой указано кардио (например «Кардио 20 минут, пульс 130»), то:
 - workoutType НЕ делай cardio
 - строку про кардио НЕ добавляй как упражнение в exercises
 - перенеси информацию про кардио в поле notes
12c. СЕТЫ И ВЕСА: если под одним упражнением несколько строк подходов и там разные веса/повторы — ОБЯЗАТЕЛЬНО используй setData (по подходам), НЕ усредняй и НЕ «переноси» последний вес на все подходы.
Пример:
Сгибания голени сидя
1x15, 14 кг
3x12, 18 кг
→ один exercises-объект, setData = [{"reps":15,"weight":14},{"reps":12,"weight":18},{"reps":12,"weight":18},{"reps":12,"weight":18}]
12d. ЕДИНИЦЫ ВЕСА: «кг» — килограммы. Если в тексте опечатка «кн», считай это «кг» (килограммы).
...
export class YandexAiService {
  ...
  static async parseWorkoutTextLikeOcr(text: string): Promise<AiWorkoutResult> {
    ...
    return this.parseWorkoutJson(result, text)
  }
