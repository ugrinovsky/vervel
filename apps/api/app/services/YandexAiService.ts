...
/** Второй проход: отфильтровать пары, где паттерн движения не совпадает. */
const VERIFY_SUGGEST_STANDARD_LINKS_PROMPT = `Тебе передан JSON с полем pairs — список предложенных связей «упражнение из журнала → эталон». Каждая запись: sourceExerciseId, standardId, exerciseName (название у кандидата), displayLabel (название эталона).

Задача: вернуть links только с теми парами, где exerciseName и displayLabel описывают ОДНО И ТО ЖЕ упражнение (одинаковый паттерн движения, положение, суть). Синонимы и разные формулировки одного движения — оставить; разные движения — убрать.

Для каждой пары мысленно (не в ответе): опиши в 1–2 фразы движение по exerciseName и отдельно по displayLabel; совпадают ли по сути? При малейшем сомнении пару исключи.

Нельзя добавлять новые пары и нельзя менять sourceExerciseId/standardId — только копировать из входа или опустить пару.

Верни ТОЛЬКО валидный JSON, без markdown:
{"links":[{"sourceExerciseId":"<как во входе>","standardId":<число>}]}

Если ни одна пара не годится — {"links":[]}.`
...
- Без markdown и без текста вокруг — только объект JSON.`
  ...
  private static extractStandardLinksFromParsed(parsed: unknown): Array<{
  ...
  static async suggestStrengthLogStandardLinks(
    ...
    const linksRaw = Array.isArray((parsed as { links?: unknown }).links)
      ? ((parsed as { links: unknown[] }).links)
      : []
    let out = this.extractStandardLinksFromParsed(parsed)
    ...
    if (out.length > 0) {
      const nameByExId = new Map(candidates.map((c) => [c.exerciseId, c.exerciseName]))
      const labelByStdId = new Map(standards.map((s) => [s.id, s.displayLabel]))
      const pairs = out.map((l) => ({
        sourceExerciseId: l.sourceExerciseId,
        standardId: l.standardId,
        exerciseName: nameByExId.get(l.sourceExerciseId) ?? '',
        displayLabel: labelByStdId.get(l.standardId) ?? '',
      }))

      logger.info(
        {
          event: 'ai:suggest-standard-links:verify-request',
          pairsCount: pairs.length,
          pairs,
        },
        'YandexAiService.suggestStrengthLogStandardLinks: verify request'
      )

      const verifyRaw = await this.callGpt(
        folderId,
        VERIFY_SUGGEST_STANDARD_LINKS_PROMPT,
      ...
      const verifyCleaned = verifyRaw.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim()
      try {
        const verifyParsed = JSON.parse(verifyCleaned)
        const verified = this.extractStandardLinksFromParsed(verifyParsed)
        const allowedKeys = new Set(out.map((l) => `${l.sourceExerciseId}\t${l.standardId}`))
        out = verified.filter((l) => allowedKeys.has(`${l.sourceExerciseId}\t${l.standardId}`))

        logger.info(
          {
            event: 'ai:suggest-standard-links:verify-response',
            verifyCleanedLength: verifyCleaned.length,
            beforeCount: allowedKeys.size,
            afterCount: out.length,
            links: out,
          },
          'YandexAiService.suggestStrengthLogStandardLinks: verify response'
        )
      } catch (e) {
        logger.warn(
          {
            event: 'ai:suggest-standard-links:verify-json-error',
            verifyCleanedLength: verifyCleaned.length,
            verifyCleanedPreview: verifyCleaned.slice(0, 800),
            err: String(e),
          },
          'YandexAiService.suggestStrengthLogStandardLinks: verify JSON failed, keeping phase-1 links'
        )
      }
    }
