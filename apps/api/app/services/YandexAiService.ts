...
const SUGGEST_STANDARD_LINKS_PROMPT = `Сопоставь названия упражнений из журнала (candidates) с эталонами пользователя (standards).

Глобального каталога упражнений нет: есть только текст поля exerciseName у кандидата и текст displayLabel у эталона. Сравнивай их напрямую.
...
- Связывай только если это по сути ОДНО И ТО ЖЕ упражнение (тот же паттерн движения, тот же вариант: снаряд, угол корпуса, хват и т.д.). Нельзя объединять разные упражнения из‑за «похожих мышц» или общих слов вроде «гантели», «наклон», «тяга», «жим».
- Примеры НЕ связывать: пуловер / пулловер ↔ разводка гантелей на наклонной; разведение гантелей лёжа ↔ разводка на наклонной; тяга штанги в наклоне ↔ румынская тяга; французский жим ↔ жим гантелей лёжа — это разные движения.
