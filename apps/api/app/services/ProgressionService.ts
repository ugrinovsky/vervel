...
function strengthLogCompositeKey(baseGk: string, workoutType: StrengthLogWorkoutType): string {
...
export class ProgressionService {
  ...
  private static resolveStandardIdForExerciseId(
    exerciseId: string,
    ctx: StandardContext
  ): number | null {
  ...
  private static oneRmForGroupKey(
    w: Workout,
    compositeGroupKey: string,
    ctx: StandardContext
  ): number {
  ...
  static async getGroupLeaderboard(
    ...
    const { computeLevel } = await import('./xp_logic.js')
  ...
  static async getStrengthLog(userId: number): Promise<StrengthLogPayload> {
    ...
    if (pinnedExerciseIds.length > 0) {
      ...
      entries.sort((a, b) => {
        ...
        return (
          (a.workoutType ? wtOrder[a.workoutType] : 9) -
          (b.workoutType ? wtOrder[b.workoutType] : 9)
        )
      })
  ...
  static async replaceStrengthLogPins(userId: number, exerciseIds: string[]): Promise<void> {
    const unique = normalizePinnedExerciseIdList(exerciseIds).map((id) =>
      ensureCompositePinnedKey(id)
    )
  ...
  private static computeDashboardMetricForGroup(
    ...
    if (best1RMLast30d !== null && best1RMPrev30d !== null && best1RMPrev30d > 0) {
      changePct = Math.round(((best1RMLast30d - best1RMPrev30d) / best1RMPrev30d) * 100 * 10) / 10
    }
  ...
  private static async buildWeightedExerciseOptionsForUser(
    ...
    const pseudoEntries: StrengthLogEntry[] = [...rawIds].map((exerciseId) => ({
      exerciseId,
      exerciseName: exerciseId.startsWith('custom:')
        ? nameHint.get(exerciseId)
          ? capitalizeFirstForDisplay(nameHint.get(exerciseId)!)
          : capitalizeFirstForDisplay(exerciseId.slice('custom:'.length).trim() || exerciseId)
   ...
   */
  static async suggestStandardAliasLinksWithAi(
    userId: number
  ): Promise<StandardLinkSuggestionDTO[]> {
